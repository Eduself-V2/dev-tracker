...
export default function Dashboard() {
  ...
  const stageConfigs = [
    ...
    {
      key: "pushedToProduction",
      label: "Production",
      value: stats.pushedToProduction ?? 0,
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row gap-4 justify-between sm:items-end">
        ...
        <div className="flex items-center gap-2 min-w-[220px]">
          ...
          <Select
            ...
            onValueChange={(val) =>
              setProjectId(val === "all" ? undefined : parseInt(val))
            }
          >
            <SelectTrigger>
      ...
      <div className="grid gap-4 md:grid-cols-5">
        {stageConfigs.map((config) => (
          ...
          >
            <CardHeader
              className={`flex flex-row items-center justify-between pb-2 space-y-0 border-b ${config.bg}`}
            >
              <CardTitle className="text-sm font-medium">
                {config.label}
              </CardTitle>
